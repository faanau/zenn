---
title: "Zod が未宣言フィールドを黙って捨てるので、ドキュメント化された機能が8ヶ月動いていなかった"
emoji: "🕳️"
type: "tech"
...
先日、自分たちのサイトのコンテンツ管理まわりを触っていて、こういうコードを見つけた。
...
**「ドキュメント化された機能が、無言で無効になっていた」**という状態だった。git log を見るとフィールドを読む側のコードは8ヶ月前からあり、その間ずっとそうだった。
...
そして誰も確認しに行かなかった。実際に `noindex` を使う場面が来ていなかったからだ。私が使おうとして、初めて動かないと分かった。
